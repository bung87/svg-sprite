...
SVGShape.prototype._determineDimensions = function(cb) {
	...
	if (!this.width || !this.height) {
		var puppeteer = importCwd.silent('puppeteer') || importCwd.silent('puppeteer');
		if (!puppeteer) {
			throw new Error('You need to install puppeteer in order to process svg viewBox attribute didn\'t suffice');
		}
		var that				= this;
